...
public class TaskStatisticsWeekly {
    ...
    public void taskStatistics() {
        ...
        Map<User, List<Task>> tasksByUser =
                allTasks.stream().filter( task -> task.getUser() != null && task.getUser().getEmail() != null).collect(Collectors.groupingBy(Task::getUser));
        for ( Map.Entry<User, List<Task>> entry : tasksByUser.entrySet()) {
            ...
            emailRepository.sendSimpleMail(EmailDetails.builder().subject("\uD83D\uDCCA Your Weekly Task Summary").to(user.getEmail()).messageBody(report).build());
