        ...
        return args -> {
          ...
          try {
              ...
              if ( roleRepository.findByName(RoleName.ADMIN.name()).isEmpty()) {
                  ...
                  roleRepository.save(new Role(RoleName.ADMIN.name(), "Permission for " + RoleName.ADMIN , permissions));
              }
              if ( userRepository.findByName(username).isEmpty()) {
                  Role adminRole = roleRepository.findByName(RoleName.ADMIN.name())
                          .orElseThrow(() -> new ApiException(ErrorCode.ROLE_NOT_FOUND));
                  User user =
                          User.builder().name(username).password(passwordEncoder.encode(password)).roles(Set.of(adminRole)).build();
