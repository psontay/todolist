...
public class TaskDeadlineNotifier {
    ...
    public void checkAndSendDeadlineWarnings() {
        ...
        for(Task task : tasks) {
            ...
            log.info("Task {} - now: {}, sendTime: {}, warningEmailSent: {}",task.getId(), now, sendTime,
                     task.getWarningEmailSent());
            if(now.isAfter(sendTime) && now.isBefore(task.getDeadline()) && Boolean.FALSE.equals(task.getWarningEmailSent())) {
