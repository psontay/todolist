...
public class RoleServiceImpl implements RoleService {
    ...
    @Caching(
            evict = {
                    @CacheEvict(cacheNames = "role-list",
                            key = "'all'"),
                    @CacheEvict(cacheNames = "role-keyword",
                            allEntries = true)
            })
    ...
    @Override
    @Cacheable(cacheNames = "role-list",
            key = "'all'")
    @PreAuthorize("hasRole('ADMIN')")
    ...
    @Override
    @Cacheable(cacheNames = "role-by-name",
            key = "#roleName")
    @PreAuthorize("hasRole('ADMIN')")
    ...
    @Override
    @Cacheable(cacheNames = "role-by-des",
            key = "#des")
    @PreAuthorize("hasRole('ADMIN')")
    ...
    @Caching(
            evict = {
                    @CacheEvict(cacheNames = "role-by-name",
                            key = "#roleName"),
                    @CacheEvict(cacheNames = "role-list",
                            key = "'all'"),
                    @CacheEvict(cacheNames = "role-keyword",
    ...
    @Override
    @Cacheable(cacheNames = "role-keyword",
            key = "#keyWord")
    @PreAuthorize("hasRole('ADMIN')")
    ...
    public RoleResponse addPermissionsToRole(String roleName, List<String> permissionNames) {
        ...
        log.debug("Permissions size before adding: {}",
                  role.getPermissions()
                      .size());
        role.getPermissions()
            .addAll(permissions);
        log.debug("Permissions size after adding: {}",
                  role.getPermissions()
                      .size());
