        ...
        return args -> {
            ...
            try {
                ...
                for (String permission : defaultPermissions) {
                    if (permissionRepository.findByName(permission)
                                            .isEmpty()) {
                        permissionRepository.save(new Permission(permission, "Permission for " + permission));
                    }
                }
                if (roleRepository.findByName(RoleName.ADMIN.name())
                                  .isEmpty()) {
                ...
                log.error("Failed to initialize application default data", e);
            }
        };
    }
